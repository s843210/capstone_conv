...
OUT_RESULT_JSON = BASE_DIR / "outputs" / "reports" / "random_forest_weather_binary_result.json"
OUT_IMPORTANCE_CSV = BASE_DIR / "outputs" / "reports" / "random_forest_weather_binary_feature_importance.csv"
OUT_SAMPLE_CSV = BASE_DIR / "outputs" / "reports" / "random_forest_weather_binary_predictions_sample.csv"
OUT_MODEL = BASE_DIR / "outputs" / "models" / "random_forest_weather_binary_model.pkl"
...
def main() -> None:
    ...
    # 8) compare with existing RF-fast
    compare = {
        "existing_rf_fast": EXISTING_RF_FAST_METRICS,
        "new_rf_weather_binary": {"mae": mae, "rmse": rmse, "r2": r2},
        "delta": {
    ...
    # 10) save model bundle (do not overwrite existing fast model)
    bundle = {
        "model": model,
    ...
    print(f"RF-weather-binary Test MAE: {mae:.6f}")
    print(f"RF-weather-binary Test RMSE: {rmse:.6f}")
    print(f"RF-weather-binary Test R2: {r2:.6f}")
