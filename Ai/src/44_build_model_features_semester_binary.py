...
OUTPUT_CSV = BASE_DIR / "data" / "processed" / "model_features_semester_binary.csv"
...
def main() -> None:
    ...
    required_cols = {"date", "plu_code", "sales_qty", *WEATHER_CONT_COLS, "is_vacation"}
    ...
    # vacation flag numeric cleanup for is_semester derivation
    df["is_vacation"] = pd.to_numeric(df["is_vacation"], errors="coerce").fillna(0)
    ...
    # is_semester feature from is_vacation
    df["is_semester"] = (df["is_vacation"] != 1).astype(int)

    # remove continuous weather columns + simplified academic columns
    drop_cols = [c for c in WEATHER_CONT_COLS + ACADEMIC_DROP_COLS if c in df.columns]
    ...
    # leakage prevention: shift(1) before rolling
    ...
    print(f"Saved model features (semester+binary weather): {OUTPUT_CSV}")
