...
def _parse_yyymmdd(value) -> pd.Timestamp:
...
def _build_academic_event_map(academic_calendar_paths: Optional[List[str]]) -> Dict[pd.Timestamp, int]:
    ...
    season_dates: Set[pd.Timestamp] = set()
    ...
    for path in academic_calendar_paths:
        ...
        if "date" not in raw.columns or "event" not in raw.columns:
            continue
        ...
        for _, row in raw.iterrows():
            ...
            dt = _parse_yyymmdd(row.get("date"))
            if pd.isna(dt):
                continue
            ...
            if "개강" in event_text and sem is not None and event_year is not None:
...
def _build_weather_frame(weather_path: Optional[str]) -> pd.DataFrame:
    ...
    date_col = "일시" if "일시" in raw.columns else ("date" if "date" in raw.columns else None)
    temp_col = "평균기온(°C)" if "평균기온(°C)" in raw.columns else None
    rain_col = "일강수량(mm)" if "일강수량(mm)" in raw.columns else None

    if date_col is None:
        return pd.DataFrame(columns=["date", "avg_temp_c", "precipitation_mm", "is_rain"])

    w = pd.DataFrame()
    w["date"] = pd.to_datetime(raw[date_col], errors="coerce").dt.normalize()
