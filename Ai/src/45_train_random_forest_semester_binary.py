...
OUT_RESULT_JSON = BASE_DIR / "outputs" / "reports" / "random_forest_semester_binary_result.json"
OUT_IMPORTANCE_CSV = BASE_DIR / "outputs" / "reports" / "random_forest_semester_binary_feature_importance.csv"
OUT_SAMPLE_CSV = BASE_DIR / "outputs" / "reports" / "random_forest_semester_binary_predictions_sample.csv"
OUT_MODEL = BASE_DIR / "outputs" / "models" / "random_forest_semester_binary_model.pkl"
...
def main() -> None:
    ...
    # 7) compare with final model
    compare = {
        "final_rf_weather_binary": FINAL_MODEL_METRICS,
        "new_rf_semester_binary": {"mae": mae, "rmse": rmse, "r2": r2},
        "delta": {
    ...
    # 9) save model bundle (do not overwrite final model)
    bundle = {
        "model": model,
    ...
    result = {
        "model": "RandomForestRegressor_semester_binary",
        "params": {
            "n_estimators": 100,
            "max_depth": 20,
            "min_samples_leaf": 3,
    ...
    print(f"RF-semester-binary Test MAE: {mae:.6f}")
    print(f"RF-semester-binary Test RMSE: {rmse:.6f}")
    print(f"RF-semester-binary Test R2: {r2:.6f}")
