...
# 데이터 위치       = capstone_conv/data
# =====================================
train_path = "../../data/rf_train_compact.csv"
test_path = "../../data/rf_test_compact.csv"
out_dir = "."  # 결과는 현재 폴더(rf_trainer)에 저장
...
def metrics(y_true, y_pred):
    ...
    return {
        ...
        "RMSE": rmse,
        "WAPE": wape,
        "ACC_1_minus_WAPE": float(1 - wape) if np.isfinite(wape) else float("nan"),
    }
...
for key, params in candidates:
    ...
    results.append({"model_key": key, "params": params, "metrics": m})

    # WAPE가 가장 낮은 모델을 best로 선택
