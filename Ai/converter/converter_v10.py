"""
converter_v10.py  ─  에러없조 | 판매현황 → RF 학습용 feature CSV 변환기
=============================================================================
[v4 → v10 개선 사항]
  1. 날씨 컬럼 매칭 강화
     - °C / ℃ / ˚C 등 온도 기호 정규화 → 컬럼명 불일치 방지
     - 숫자 전용 컬럼(지점번호 등)이 기온으로 오매칭되는 문제 수정
     - 기온 컬럼 후보에 '평균기온(℃)' 추가
  2. 날씨 CSV 인코딩 감지 개선
     - _read_csv_with_fallback에서 cp949를 utf-8-sig보다 먼저 시도
     - 한국 기상청 데이터의 cp949 인코딩 우선 처리
  3. 학사일정-날씨-판매 날짜 매칭 검증 로그 추가
     - 날씨 데이터 누락 날짜 경고 출력
     - 학사일정 매칭 결과 요약 출력
  4. 외부 데이터 파일 누락 시 "조용한 실패" 제거
     - 학사일정/날씨 파일 경로가 틀리면 크게 경고 출력
     - 이전: 파일 없으면 무시하고 academic_event=0, avg_temp_c=NaN 반환
     - 이후: 명확한 경고로 사용자가 즉시 인지 가능
  5. academic_event 코드 체계 개편
     - 0: 비학기  1: 정규학기(개강~종강 전체)
     - 2: 시험기간  3: 축제  4: 계절학기
     - 우선순위: 시험 > 축제 > 계절학기 > 정규학기
  6. 축제 키워드 자동 감지
     - '축제', '대동제', 'festival' 키워드로 축제 이벤트 자동 분류
     - 학사일정 CSV에 축제 행 추가 시 즉시 반영됨
...
[사용법]
  터미널
    python converter_v10.py --input data\\raw\\240501-15 --master csv_상품분류기준표
    python converter_v10.py --input data\\raw\\240501-15           # 분류기준표 없이도 동작
  Colab
    from converter_v10 import run
...
OUTPUT_COLS = [
    ...
    "academic_event", "avg_temp_c", "precipitation_mm", "is_rain",
    "building_headcount",
    "safety_stock",
    "match_type",
]
...
def parse_date_from_foldername(name: str, default_year: Optional[int] = None) -> Optional[date]:
    """Extract date from folder name. Supports YYMMDD and MMDD(with default_year)."""
...
def load_master(master_path: str) -> Tuple[Dict, Dict, List[str]]:
    ...
    print(f"  [OK] 분류기준표 로드 완료: {total_raw:,}개 -> 중복 제거 후 {len(master):,}개")
...
def collect_all_sales(input_dirs: List[str]) -> pd.DataFrame:
    ...
    for input_dir in input_dirs:
        ...
        default_year = None
        m_input = re.search(r"(\d{2})(\d{2})", input_path.name)
        if m_input:
            default_year = 2000 + int(m_input.group(1))
        ...
        for subdir in subdirs:
            ...
            if is_month_folder(subdir):
                ...
                for day_dir in day_dirs:
                    folder_date = parse_date_from_foldername(day_dir.name, default_year=default_year)
            ...
            folder_date = parse_date_from_foldername(subdir.name, default_year=default_year)
            if folder_date is None:
                continue
    ...
    print(f"  [OK] 전체 수집: {len(combined):,}행 | {len(processed_dates)}일 ({date_range})")
    return combined
...
def _read_csv_with_fallback(path: str) -> pd.DataFrame:
    """Try common encodings to load CSV generated in mixed Korean environments.
    cp949를 먼저 시도하여 한국 기상청 등 cp949 데이터의 깨짐 방지."""
    last_error = None
    for enc in ("cp949", "utf-8-sig", "euc-kr", "utf-8"):
        try:
            ...
            last_error = e
    raise ValueError(f"CSV 로드 실패: {path} ({last_error})")


def _parse_flexible_date(value) -> pd.Timestamp:
    if value is None or (isinstance(value, float) and pd.isna(value)):
        return pd.NaT

    # Handle compact numeric dates first (e.g., 240422 / 20240422).
    s_compact = re.sub(r"[^\d]", "", str(value))
    if len(s_compact) == 8:
        yyyy, mm, dd = int(s_compact[:4]), int(s_compact[4:6]), int(s_compact[6:8])
        try:
            return pd.Timestamp(year=yyyy, month=mm, day=dd).normalize()
        except ValueError:
            return pd.NaT
    if len(s_compact) == 6:
        yy, mm, dd = int(s_compact[:2]), int(s_compact[2:4]), int(s_compact[4:6])
        try:
            return pd.Timestamp(year=2000 + yy, month=mm, day=dd).normalize()
        except ValueError:
            return pd.NaT
...
def _normalize_col_key(s: str) -> str:
    """컬럼명 비교용 정규화: 온도 기호 통일 + 소문자 + 공백 제거."""
    s = str(s).strip().lower()
    # °C, ℃, ˚C 등 다양한 온도 기호를 통일
    s = re.sub(r"[°℃˚]\s*c", "°c", s)
    s = s.replace("℃", "°c")
...
def _find_first_column(columns: List[str], candidates: List[str]) -> Optional[str]:
    col_map = {_normalize_col_key(c): c for c in columns}
...
def _detect_date_column_by_values(df: pd.DataFrame) -> Optional[str]:
    if df.empty:
        return None
    best_col = None
    best_score = 0.0
    for col in df.columns:
        parsed = df[col].apply(_parse_flexible_date)
...
def _detect_event_text_column(df: pd.DataFrame, date_col: Optional[str]) -> Optional[str]:
    if df.empty:
        return None
    best_col = None
    best_score = -1.0
    for col in df.columns:
        ...
        non_empty = s[s.ne("") & s.ne("nan") & s.ne("None")]
        if non_empty.empty:
            continue
        score = float(non_empty.str.len().mean()) + float(non_empty.nunique() / max(len(non_empty), 1))
        if score > best_score:
            best_score = score
            best_col = col
    return best_col
...
def _expand_date_ranges(date_set: Set[pd.Timestamp]) -> Set[pd.Timestamp]:
    if not date_set:
        return set()
    start = min(date_set)
    end = max(date_set)
    return {d.normalize() for d in pd.date_range(start=start, end=end, freq="D")}


def _expand_grouped_ranges(grouped_dates: Dict[Tuple, Set[pd.Timestamp]]) -> Set[pd.Timestamp]:
    """
    Expand date ranges per logical group to avoid accidental cross-year filling.
    Each group is expanded independently from min(date) to max(date).
    """
    expanded: Set[pd.Timestamp] = set()
    for _, dates in grouped_dates.items():
        if not dates:
            continue
        start = min(dates)
        end = max(dates)
        ...
        for day in pd.date_range(start=start, end=end, freq="D"):
            expanded.add(day.normalize())
    return expanded


def _extract_event_year(event_text: str) -> Optional[int]:
    m = re.search(r"(20\d{2})\s*(?:학년도|년도)", event_text)
    if not m:
        return None
    return int(m.group(1))


def _extract_semester(event_text: str) -> Optional[int]:
    m = re.search(r"([12])\s*학기", event_text)
    if not m:
        return None
    return int(m.group(1))


def _is_opening_event(event_text: str) -> bool:
    return "\uac1c\uac15" in event_text


def _is_closing_event(event_text: str) -> bool:
    return "\uc885\uac15" in event_text


def _is_midterm_event(event_text: str) -> bool:
    return "\uc911\uac04\uace0\uc0ac" in event_text


def _is_final_event(event_text: str) -> bool:
    return "\uae30\ub9d0\uace0\uc0ac" in event_text


def _is_season_event(event_text: str) -> bool:
    return "\uacc4\uc808\ud559\uae30" in event_text


def _is_summer_season(event_text: str) -> bool:
    return ("\ud558\uacc4" in event_text) or ("\uc5ec\ub984" in event_text)


def _is_winter_season(event_text: str) -> bool:
    return ("\ub3d9\uacc4" in event_text) or ("\uaca8\uc6b8" in event_text)


def _is_festival_event(event_text: str) -> bool:
    """축제 이벤트 여부 판별. '축제', 'festival', '대동제' 등 키워드."""
    lowered = event_text.lower()
    return (
        "\ucd95\uc81c" in event_text          # 축제
        or "\ub300\ub3d9\uc81c" in event_text  # 대동제
        or "festival" in lowered
    )


def _infer_semester_from_date(dt: pd.Timestamp) -> int:
    return 2 if dt.month >= 8 else 1


def _build_academic_event_map(academic_calendar_paths: Optional[List[str]]) -> Dict[pd.Timestamp, int]:
    """
    academic_event rule:
      0: 비학기 (기본값)
      1: 정규학기 (개강일 ~ 종강일 전체)
    ...
    우선순위 (덮어쓰기 순서):
      1 (정규학기) → 4 (계절학기) → 3 (축제) → 2 (시험)
      즉, 시험이 가장 높은 우선순위. 같은 날에 여러 이벤트가 있으면
      시험 > 축제 > 계절학기 > 정규학기 순으로 결정된다.
    """
    if not academic_calendar_paths:
        return {}

    midterm_groups: Dict[Tuple[int, int], Set[pd.Timestamp]] = {}
    final_groups: Dict[Tuple[int, int], Set[pd.Timestamp]] = {}
    summer_groups: Dict[int, Set[pd.Timestamp]] = {}
    winter_groups: Dict[int, Set[pd.Timestamp]] = {}
    season_generic_groups: Dict[int, Set[pd.Timestamp]] = {}
    festival_groups: Dict[int, Set[pd.Timestamp]] = {}
    sem_start: Dict[Tuple[int, int], pd.Timestamp] = {}
    sem_end: Dict[Tuple[int, int], pd.Timestamp] = {}
    sem_any_dates: Dict[Tuple[int, int], List[pd.Timestamp]] = {}

    loaded_files = 0
    total_rows = 0
    for path in academic_calendar_paths:
        p = str(path).strip()
        if not p:
            continue
        if not Path(p).exists():
            # 조용히 넘어가지 않고 경고를 크게 출력
            print(f"  ⚠ [학사일정] 파일을 찾을 수 없음: {p}")
            print(f"     → 경로/파일명 확인 (공백/괄호 포함 시 큰따옴표로 감싸기)")
            continue

        try:
            raw = _read_csv_with_fallback(p)
        except Exception as e:
            print(f"  ⚠ [학사일정] 읽기 실패: {p} ({e})")
            continue
        if raw.empty:
            print(f"  ⚠ [학사일정] 비어있는 파일: {p}")
            continue

        date_col = _find_first_column(
            list(raw.columns),
            ["date", "일자", "일시", "날짜", "학사일자", "학사일정일자", "dt"],
        )
        event_col = _find_first_column(
            list(raw.columns),
            ["event", "학사일정", "일정", "내용", "행사", "description", "title"],
        )
        if date_col is None:
            date_col = _detect_date_column_by_values(raw)
        if event_col is None:
            event_col = _detect_event_text_column(raw, date_col)
        if date_col is None or event_col is None:
            print(f"  ⚠ [학사일정] date/event 컬럼 탐지 실패: {p} (cols={list(raw.columns)})")
            continue

        loaded_files += 1
        file_rows = 0
        for _, row in raw.iterrows():
            event_text = str(row.get(event_col, "")).strip()
            if not event_text:
                continue

            dt = _parse_flexible_date(row.get(date_col))
            if pd.isna(dt):
                continue

            file_rows += 1
            sem = _extract_semester(event_text)
            event_year = _extract_event_year(event_text)
            key_year = event_year if event_year is not None else dt.year
            opening = _is_opening_event(event_text)
            closing = _is_closing_event(event_text)
            sem_for_term = sem
            if sem_for_term is None and (opening or closing):
                sem_for_term = _infer_semester_from_date(dt)

            # Guard against obvious opening-date typo (e.g., 260901 but event says 2025 2학기 개강).
            if opening and sem_for_term is not None and event_year is not None:
                if dt.year == event_year + 1 and dt.month >= 9:
                    dt = dt.replace(year=event_year)
                key_year = event_year

            sem_key_for_exam = sem if sem is not None else 0
            if _is_midterm_event(event_text):
                key = (key_year, sem_key_for_exam)
                midterm_groups.setdefault(key, set()).add(dt)
            if _is_final_event(event_text):
                key = (key_year, sem_key_for_exam)
                final_groups.setdefault(key, set()).add(dt)
            if _is_festival_event(event_text):
                festival_groups.setdefault(key_year, set()).add(dt)
            if _is_season_event(event_text):
                if _is_summer_season(event_text):
                    summer_groups.setdefault(key_year, set()).add(dt)
                elif _is_winter_season(event_text):
                    winter_groups.setdefault(key_year, set()).add(dt)
                else:
                    season_generic_groups.setdefault(key_year, set()).add(dt)

            if sem_for_term is not None:
                key = (key_year, sem_for_term)
                sem_any_dates.setdefault(key, []).append(dt)
                if opening:
                    sem_start[key] = min(dt, sem_start[key]) if key in sem_start else dt
                if closing:
                    sem_end[key] = max(dt, sem_end[key]) if key in sem_end else dt
        total_rows += file_rows
        print(f"  [OK] 학사일정 로드: {Path(p).name} ({file_rows}행)")

    if loaded_files == 0:
        print(f"  ⚠ 학사일정 파일이 1개도 로드되지 않았습니다 → academic_event=0 전체")
        return {}

    status_by_date: Dict[pd.Timestamp, int] = {}

    # 우선순위 낮은 것부터 덮어쓰기:
    # 1. 정규학기 → 1
    for key, all_dates in sem_any_dates.items():
        start = sem_start.get(key, min(all_dates))
        end = sem_end.get(key, max(all_dates))
        if start > end:
            start, end = end, start
        for day in pd.date_range(start=start, end=end, freq="D"):
            status_by_date[day.normalize()] = 1

    # 2. 계절학기 → 4
    season_dates = _expand_grouped_ranges(summer_groups)
    season_dates |= _expand_grouped_ranges(winter_groups)
    season_dates |= _expand_grouped_ranges(season_generic_groups)
    for day in season_dates:
        status_by_date[day.normalize()] = 4

    # 3. 축제 → 3
    festival_dates = _expand_grouped_ranges(festival_groups)
    for day in festival_dates:
        status_by_date[day.normalize()] = 3

    # 4. 시험 → 2 (최고 우선순위)
    exam_dates = _expand_grouped_ranges(midterm_groups) | _expand_grouped_ranges(final_groups)
    for day in exam_dates:
        status_by_date[day.normalize()] = 2

    print(f"  [OK] 학사일정 매핑 완료: {loaded_files}개 파일, {total_rows}행 → {len(status_by_date)}일")
    return status_by_date


def _build_weather_frame(weather_path: Optional[str]) -> pd.DataFrame:
    empty = pd.DataFrame(columns=["date", "avg_temp_c", "precipitation_mm", "is_rain"])
    if not weather_path:
        return empty
    p = str(weather_path).strip()
    if not p:
        return empty
    if not Path(p).exists():
        # 조용히 넘어가지 않고 경고를 크게 출력
        print(f"  ⚠ [날씨] 파일을 찾을 수 없음: {p}")
        print(f"     → 경로/파일명 확인 (공백/괄호 포함 시 큰따옴표로 감싸기)")
        return empty
    ...
    try:
        ...
        print(f"  ⚠ [날씨] 읽기 실패: {p} ({e})")
        return empty
    if raw.empty:
        print(f"  ⚠ [날씨] 비어있는 파일: {p}")
        return empty
    print(f"  [OK] 날씨 로드: {Path(p).name} ({len(raw)}행, cols={list(raw.columns)})")

    date_col = _find_first_column(
        list(raw.columns),
        ["일시", "날짜", "일자", "date", "datetime", "dt"],
    )
    if date_col is None:
        date_col = _detect_date_column_by_values(raw)
    temp_col = _find_first_column(
        list(raw.columns),
        [
            "평균기온(°c)", "평균기온(℃)", "평균기온",
            "일평균기온", "temperature", "temp_c",
            "avg_temp_c", "mean_temp",
        ],
    )
    rain_col = _find_first_column(
        list(raw.columns),
        [
            "일강수량(mm)", "강수량(mm)", "일강수량", "강수량",
            "precipitation_mm", "precip_mm", "rain_mm",
        ],
    )
    rain_flag_col = _find_first_column(
        list(raw.columns),
        ["강수여부", "강수유무", "비여부", "rain", "is_rain", "rain_flag"],
    )

    if date_col is None:
        return pd.DataFrame(columns=["date", "avg_temp_c", "precipitation_mm", "is_rain"])

    w = pd.DataFrame()
    # Reuse flexible parser so values like 250424 (YYMMDD) are handled safely.
    w["date"] = raw[date_col].apply(_parse_flexible_date)

    # 날짜 컬럼과 지점/지역명 등 비-기상 컬럼을 제외한 숫자 컬럼만 후보로 사용
    _skip_cols = {date_col}
    _skip_col_keywords = {"지점", "지역", "station", "stn", "code", "id"}
    numeric_cols = []
    for c in raw.columns:
        if c in _skip_cols:
            continue
        nk = _normalize_col_key(c)
        if any(kw in nk for kw in _skip_col_keywords):
            continue
        if pd.to_numeric(raw[c], errors="coerce").notna().mean() >= 0.5:
            # 값의 분산이 거의 없는 컬럼(지점번호 등)은 제외
            vals = pd.to_numeric(raw[c], errors="coerce").dropna()
            if len(vals) > 0 and vals.std() > 0.1:
                numeric_cols.append(c)
    if temp_col is None and numeric_cols:
        temp_col = numeric_cols[0]
    if rain_col is None and rain_flag_col is None and len(numeric_cols) >= 2:
        rain_col = numeric_cols[1]
    if temp_col is not None:
        w["avg_temp_c"] = pd.to_numeric(raw[temp_col], errors="coerce")
    else:
        w["avg_temp_c"] = pd.NA
    if rain_col is not None:
        w["precipitation_mm"] = pd.to_numeric(raw[rain_col], errors="coerce").fillna(0.0)
    elif rain_flag_col is not None:
        rain_flag = raw[rain_flag_col].astype(str).str.strip().str.lower()
        rain_yes = rain_flag.isin({"1", "y", "yes", "true", "t", "rain", "비", "있음"})
        w["precipitation_mm"] = rain_yes.astype(int).astype(float)
    else:
    ...
    weather_df = (
        w.groupby("date", as_index=False)
         .agg({"avg_temp_c": "mean", "precipitation_mm": "mean"})
    )
    weather_df["is_rain"] = (weather_df["precipitation_mm"] > 0).astype(int)
    return weather_df


def add_calendar_features(
    df: pd.DataFrame,
    academic_calendar_paths: Optional[List[str]] = None,
    weather_path: Optional[str] = None,
) -> pd.DataFrame:
    """
    캘린더 + 학사일정 + 날씨 피처를 날짜 기준으로 매칭한다.
    """
    df = df.copy()
    ...
    academic_map = _build_academic_event_map(academic_calendar_paths)
    df["academic_event"] = dt.dt.normalize().map(academic_map).fillna(0).astype(int)

    # 학사일정 매칭 결과 검증 로그
    sale_dates = set(dt.dt.normalize().dropna().unique())
    if academic_map:
        matched_academic = sale_dates & set(academic_map.keys())
        ac_counts = {}
        for d in matched_academic:
            v = academic_map[d]
            ac_counts[v] = ac_counts.get(v, 0) + 1
        label_map = {0: "비학기", 1: "정규학기", 2: "시험기간", 3: "축제", 4: "계절학기"}
        print(f"  ┌─ 학사일정 매칭 검증 {'─'*30}")
        print(f"  │  판매 데이터 일수: {len(sale_dates)}일")
        print(f"  │  학사일정 매칭됨 : {len(matched_academic)}일")
        no_academic = sale_dates - set(academic_map.keys())
        print(f"  │  학사일정 미매칭 : {len(no_academic)}일 (academic_event=0)")
        for code, cnt in sorted(ac_counts.items()):
            print(f"  │    {label_map.get(code, f'코드{code}')}: {cnt}일")
        print(f"  └─{'─'*50}")

    weather_df = _build_weather_frame(weather_path)
    if weather_df.empty:
        df["avg_temp_c"] = pd.NA
        ...
        # 날씨 데이터 매칭 검증 로그
        weather_dates = set(weather_df["date"].dropna().unique())
        no_weather = sale_dates - weather_dates
        if no_weather:
            sorted_missing = sorted(no_weather)
            print(f"  ⚠ 날씨 데이터 누락: {len(no_weather)}일 (avg_temp_c=NaN, precipitation_mm=0)")
            if len(sorted_missing) <= 5:
                for d in sorted_missing:
                    print(f"    - {d.strftime('%Y-%m-%d')}")
            else:
                print(f"    {sorted_missing[0].strftime('%Y-%m-%d')} ~ {sorted_missing[-1].strftime('%Y-%m-%d')}")
        else:
            print(f"  [OK] 날씨 데이터: 판매 기간 전체 매칭 완료 ({len(sale_dates)}일)")

    df["building_headcount"] = 0  # placeholder
    return df
...
def finalize_output(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "academic_event" in df.columns:
        df["academic_event"] = pd.to_numeric(df["academic_event"], errors="coerce").fillna(0).astype(int)
    if "avg_temp_c" in df.columns:
        df["avg_temp_c"] = pd.to_numeric(df["avg_temp_c"], errors="coerce").round(2)
    if "precipitation_mm" in df.columns:
        df["precipitation_mm"] = pd.to_numeric(df["precipitation_mm"], errors="coerce").fillna(0.0).round(2)
    if "is_rain" in df.columns:
        df["is_rain"] = pd.to_numeric(df["is_rain"], errors="coerce").fillna(0).astype(int)
...
def run(
    ...
    academic_calendar_paths : 학사일정 CSV 경로 리스트
    weather_path    : 날씨 CSV 경로
    """
    print("=" * 62)
    print("  에러없조 | 판매현황 → RF 학습 데이터 변환기 v10")
    print("=" * 62)
    ...
    if isinstance(academic_calendar_paths, str):
        academic_calendar_paths = [
            p.strip() for p in academic_calendar_paths.split(",") if p.strip()
        ]
    academic_calendar_paths = academic_calendar_paths or []

    if academic_calendar_paths:
        print(f"  학사일정   : {', '.join(academic_calendar_paths)}")
    else:
        print("  학사일정   : 없음 (academic_event=0)")
    print(f"  날씨데이터 : {weather_path if weather_path else '없음 (기본값 사용)'}")
    ...
    feature_df = add_calendar_features(
        feature_df,
        academic_calendar_paths=academic_calendar_paths,
...
def main():
    if _is_interactive():
        ...
        print("from converter_v10 import run")
        print("run(input_dir='data/raw/240501-15', master_dir='csv_상품분류기준표')")
        return

    parser = argparse.ArgumentParser(
        description="에러없조 | 판매현황 → RF 학습 데이터 변환기 v10",
...
사용 예시:
  ...
  python converter_v10.py --input data\\raw\\240501-15

  # 여러 폴더 동시 처리
  python converter_v10.py --input "data\\raw\\240501-15,data\\raw\\250601-30" --master csv_상품분류기준표

  # 출력 폴더 지정
  python converter_v10.py --input data\\raw\\240501-15 --master csv_상품분류기준표 --output data\\processed
        """
    )
    ...
    run(
        ...
        academic_calendar_paths=args.academic_calendars,
