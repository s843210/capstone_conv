...
=============================================================================
[v2 → v3 개선 사항]
  1. UserWarning 제거 : master.__fuzzy_lookup 방식 → 일반 변수로 반환
  2. 미매칭 중복 출력 수정 : 날짜별 중복 → 고유 상품명 기준으로 리포트
  3. --master 선택사항으로 변경 : 없으면 product_name 기반으로 동작
  4. 시간대 피처 제외 이유 주석 명시
  5. 전체 주석 및 로그 가독성 개선
...
def is_ambiguous_subtotal(name: str) -> bool:
    """
    is_subtotal_row()를 통과했지만 소계행으로 의심되는 행 추가 판별.
    sales == 0 일 때만 호출하는 보조 함수.

    [판별 원리]
    이마트24 시스템에서 소계행은 하위 항목이 펼쳐지지 않으면 sales=0으로만 보임.
    소계행은 '카테고리명(용기/규격/분류)' 형태로 괄호 안에 숫자가 없음.
    실제 상품은 '브랜드)상품명(용량/중량/수량)' 형태로 괄호 안에 숫자가 있음.

    소계 예시: 냉장커피(펫), 흰우유(중/대), 커피음료(RTD), 슈퍼키친(밑반찬)
    실제 상품: 크라운)마이쮸복숭아44g, 서울)흰우유500ml, 허쉬)초콜릿칩싱글쿠키50g(10)

    [규칙]
    상품명 끝이 '(한글 또는 영문대문자)' 패턴이고
    상품명 전체에 숫자가 없으면 → 소계로 판단
    """
    # 상품명 끝이 (한글), (영문대문자), (한글/한글) 패턴인지 확인
    m = re.search(r"\(([가-힣]+|[A-Z]+|[가-힣]+/[가-힣]+)\)$", name.strip())
    if m:
        # 상품명 전체에 숫자(용량, 중량, 수량 등)가 없으면 소계
...
def load_sales_file(path: str) -> pd.DataFrame:
    ...
    2단계: is_ambiguous_subtotal() - 1단계 통과한 애매한 소계행 추가 제거
           (sales=0 이고 괄호 안이 숫자 없는 순수 텍스트인 경우)
           예: 냉장커피(펫), 흰우유(중/대), 커피음료(RTD), 슈퍼키친(밑반찬)
           단, 진짜 상품 sales=0 (예: 크라운)마이쮸복숭아44g)은 보존
    """
    ext = Path(path).suffix.lower()
    try:
        if ext in (".xlsx", ".xls"):
    ...
    for _, row in data.iterrows():
        ...
        # 2단계: sales=0 인 애매한 소계행 추가 제거
        # (진짜 상품이 sales=0인 경우는 보존됨 — 숫자 포함 여부로 구분)
        if sales_val == 0 and is_ambiguous_subtotal(name):
            continue
...
def run(
    ...
    """
    print("=" * 62)
    print("  에러없조 | 판매현황 → RF 학습 데이터 변환기 v3")
    print("=" * 62)
